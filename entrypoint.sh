...
echo "INPUT_EXCLUDE_GLOB: ${INPUT_EXCLUDE_GLOB}"
...
if [ -n "$INPUT_EXCLUDE" ]; then
  for exl in $INPUT_EXCLUDE ; do
    MIRROR_COMMAND="${MIRROR_COMMAND} --exclude ${exl}"
  done
fi

if [ -n "$INPUT_EXCLUDE_GLOB" ]; then
  for exl in $INPUT_EXCLUDE_GLOB ; do
    MIRROR_COMMAND="${MIRROR_COMMAND} --exclude-glob ${exl}"
  done
fi
